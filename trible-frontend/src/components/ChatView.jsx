...
export default function ChatView({ messages, input, setInput, sendMessage, activeModule, activeScenario }) {
  const modules = [
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-between p-6 h-screen">
      <h2 className="text-xl font-bold mb-4">
        {modules[activeModule].scenarios[activeScenario] || "Select a Scenario"}
      </h2>
      ...
      <div className="mt-4 flex w-full max-w-2xl sticky bottom-4 bg-gray-900 p-4 rounded-lg">
        ...
        />
        <Button className="ml-3" onClick={sendMessage}>
          Send
        </Button>
      </div>
    </div>
